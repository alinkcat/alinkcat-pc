...
/// Save text content to the specified path (for log exports and similar uses).
#[tauri::command]
...
/// Get the unique device identifier (persisted at ~/.ilinkcat/device_code and unaffected by browser-cache clearing).
#[tauri::command]
...
/// Make a generic API request through the Rust backend to bypass CORS.
/// Supports GET/POST/PUT/DELETE, a JSON body, and custom headers.
#[tauri::command]
pub async fn api_request(
    ...
    // Default browser identification headers
    ...
    // try JSON parse, fall back to raw text on failure
