...
pub fn open(id: &str) -> Result<(), String> {
    let items = load();
    let item = items.iter().find(|i| i.id == id).ok_or_else(|| "launcher item not found".to_string())?;
    launch(&item.path)
}

/// Launch an application directly by path (without a registered launcher item).
