...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            // Record the Tauri resource directory for built-in theme fallback and future built-in theme support.
            // Note: do not call init_themes to automatically seed built-in themes into the user directory—
            // built-in themes are not enabled yet to avoid creating empty themes without resources.
            theme::manager::set_resource_dir(app.path().resource_dir().ok());

            // Start the local OAuth callback server for authorization callbacks from the system browser.
            services::oauth_callback_server::init_oauth_callback_server();

            // Start the WebSocket service automatically when the application launches.
            ...
            // System tray: minimize to the tray when the window is closed.
        ...
        .on_window_event(|window, event| {
            if let tauri::WindowEvent::CloseRequested { api, .. } = event {
                // Prevent closing and hide the window in the tray instead.
                api.prevent_close();
                let _ = window.hide();
            }
        })
