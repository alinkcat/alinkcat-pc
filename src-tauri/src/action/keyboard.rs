...
fn parse_key(name: &str) -> Option<Key> {
    ...
    match lower.as_str() {
        ...
        // In enigo 0.2, `Key::Insert` exists only on Windows/Linux (macOS keyboards have no Insert key).
        #[cfg(any(target_os = "windows", all(unix, not(target_os = "macos"))))]
        "insert" => Some(Key::Insert),
        // On macOS, a PC keyboard's Insert maps to Help; use Help as a fallback to avoid parse failure.
        #[cfg(target_os = "macos")]
        "insert" => Some(Key::Help),
        "help" => Some(Key::Help),
        "f1" => Some(Key::F1),
        "f2" => Some(Key::F2),
        "f3" => Some(Key::F3),
