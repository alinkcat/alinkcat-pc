...
export default function Themes() {
  ...
  // manually refresh the theme list，show a toast on success
  ...
  const handleActivate = async (id: string) => {
    try {
      ...
      setActiveId(id);
      // refresh the list immediately，so the active state shows correctly
      fetchData();
      // broadcast theme.switch to all connected phones
      try {
        ...
        // WebSocket service not running — ignore broadcast errors
      }
  ...
  const handleSyncAll = async (theme: ThemeSummary) => {
    ...
    try {
      ...
      for (const device of devices) {
        try {
          // push the theme to each connected device one by one（reuses the existing push_theme_to_device protocol）
