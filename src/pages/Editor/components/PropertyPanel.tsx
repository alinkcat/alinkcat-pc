...
// ─── weather widget: self-hosted weather API default config fallback（no login required，X-API-Key auth，key injected by the Android client） ─
// backend API spec:GET /api/public/weather?q=${city}&f=wttr（replaced with when Android locates ?g=lng,lat&f=wttr）
// returns wttr.in j1 format（current_condition[].temp_C、weatherDesc etc.），render logic unchanged
...
function WidgetProperties() {
  ...
  /** switch preset template: when weather is selected, auto-fill common defaults (URL / mapping); user only needs to fill in city and key */
  ...
  return (
    <Form layout="vertical" size="small">
      ...
      {/* z-order control (free mode only) */}
      {!isGrid && (
        <div style={{ borderTop: '1px solid #3a3a3a', paddingTop: 10, marginTop: 4 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <Button size="small" onClick={async () => {
              try {
                ...
                } catch { /* on icon extraction failure, ignore and keep the default placeholder */ }
      ...
      {widget.type === 'date' && (<>
        ...
        <Form.Item label={t('editor.propertyPanel.date.dateFormat')}>
          <Select
            value={(v.dateFormat as string) || 'YYYY-MM-DD WWW'}
            onChange={val => up('dateFormat', val)}
            options={[
              { label: t('editor.propertyPanel.date.format1'), value: 'YYYY-MM-DD WWW' },
              { label: t('editor.propertyPanel.date.format2'), value: 'YYYY-MM-DD WWW' },
              { label: t('editor.propertyPanel.date.format3'), value: 'MM-DD WWW' },
              { label: t('editor.propertyPanel.date.format4'), value: 'YYYY-MM-DD' },
              { label: t('editor.propertyPanel.date.format5'), value: 'MM/DD/YYYY' },
            ]} />
        </Form.Item>
        <Form.Item label={t('editor.propertyPanel.date.customFormat')}>
          <Input value={(v.dateFormat as string) || 'YYYY-MM-DD WWW'} onChange={e => up('dateFormat', e.target.value)} placeholder={t('editor.propertyPanel.date.customPlaceholder')} />
        </Form.Item>
      ...
      {/* widget linkage config (common to all widgets) */}
      <div style={{ borderTop: '1px solid #3a3a3a', paddingTop: 10, marginTop: 4 }}>
