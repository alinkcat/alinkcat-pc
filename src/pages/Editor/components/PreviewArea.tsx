...
function buildBgStyle(page: EditorPage): React.CSSProperties {
  ...
  // backgroundColor may be a solid color（#0a0e1a）or a CSS gradient（linear-gradient(...)）：
  // gradients must go into background / background-image，background-color does not accept gradients
  ...
  if (page.backgroundImage) {
    // when an image is present, layer it over the bg color/gradient（gradient + image coexist：image takes priority，keep the base visual）
    s.backgroundImage = `url(${page.backgroundImage})`;
    const m = (page.backgroundMode || 'cover') as string;
    // 'tile' used by the transport/mobile layer（the editor uses 'repeat'），the two are equivalent，both treated as tile
