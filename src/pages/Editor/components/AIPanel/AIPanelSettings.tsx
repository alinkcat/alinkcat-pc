...
export default function AIPanelSettings() {
  ...
  const handleSave = async () => {
    try {
      ...
      // defaultModel must be in the saved model list, otherwise Select shows a value not in options
  ...
  const handleFetchModels = async () => {
    ...
    try {
      ...
      let ids: string[];
      if (isTauri()) {
        // Tauri env: forwarded via Rust backend, bypass CORS (same as settings page)
