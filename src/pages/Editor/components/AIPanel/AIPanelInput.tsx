...
const PRESETS: { key: string; prompt: string }[] = [
  {
    ...
    prompt: 'Create a system monitoring theme with CPU monitor, memory monitor, disk monitor, network monitor, clock and date widgets, dark background',
  },
  {
    key: 'editor.aiPanel.presetClock',
    prompt: 'Create a minimal clock theme, large clock centered, date below, notes below that, light background',
  },
  {
    key: 'editor.aiPanel.presetCyberpunk',
    prompt: 'Create a cyberpunk-style theme, neon purple and cyan palette, with CPU gauge, memory monitor, clock, date, network monitor, dark purple background',
  },
  {
...
export default function AIPanelInput() {
  ...
  const handleSend = () => {
    ...
    if (droppedImages.length > 0) {
      const refs = droppedImages.map((d) => `[image:${d.name}]`).join(' ');
      final = content ? `${content}\n\nuploaded images: ${refs}` : `uploaded images: ${refs}`;
    }
    sendMessage(final);
    setText('');
  };
  ...
  return (
    ...
    >
          ...
          <button
            ...
            onClick={() => {
              if (preset.prompt) {
                ...
                // custom: focus the input for the user to type
      ...
      {/* uploaded image preview（large view） */}
