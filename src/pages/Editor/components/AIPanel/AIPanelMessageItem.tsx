...
/** 将 AI 回复按 ```json 围栏拆分为 [文本, JSON, 文本, ...]，JSON 段默认折叠。
 *  streaming=true 时 JSON 段以展开的 <pre> 渲染（不做折叠），避免流式过程中
 *  <details> 因内容增长反复重建导致折叠状态被重置的抖动。 */
...
/** 生成一条指令的人类可读目标摘要（如 "CPU 监控"、"第 0 页"、"主题赛博朋克"） */
function describeTarget(instr: AIInstruction): string {
  ...
  switch (instr.type) {
    case 'add_widget': {
      ...
      return instr.pageIndex !== undefined ? `第 ${instr.pageIndex} 页 · ${label}` : label;
    }
    case 'delete_widget':
    case 'update_widget':
    ...
    case 'resize_widget': {
      ...
      return instr.pageIndex !== undefined && instr.type === 'delete_widget' ? `第 ${instr.pageIndex} 页 · ${id}` : id;
    }
    case 'add_page':
    ...
    case 'batch':
      return `${Array.isArray(p?.actions) ? (p!.actions as unknown[]).length : 0} 个子操作`;
