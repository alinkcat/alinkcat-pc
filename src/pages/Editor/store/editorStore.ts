...
export function makeWidget(type: string, col: number, row: number): EditorWidget {
  ...
  switch (type) {
    ...
    case 'text': return { ...base, content: 'Double-click to edit text', fontSize: 16, fontWeight: 'normal', color: '#333333', textAlign: 'left', backgroundColor: 'transparent', backgroundOpacity: 0, padding: 4, borderRadius: 0 } as unknown as EditorWidget;
    ...
    case 'media-control': return { ...base, label: 'Media control', displayMode: 'always', showCover: true, showProgress: true } as unknown as EditorWidget;
    case 'system-monitor': return { ...base, label: 'System monitor', showCPU: true, showMemory: true, showDisk: true, showNetwork: true, refreshInterval: 2 } as unknown as EditorWidget;
    case 'quick-action': return { ...base, label: 'Quick Panel', columns: 2, rows: 2, cells: [
      { type: 'launcher', name: 'Calculator', path: 'calc' },
      { type: 'launcher', name: 'Notepad', path: 'notepad' },
      { type: 'snippet', title: 'Welcome', snippets: [{ id: 's1', label: 'Hello', content: 'Hello, welcome!' }] },
      { type: 'snippet', title: 'Closing', snippets: [{ id: 's2', label: 'Thanks', content: 'Thank you for your visit!' }] },
    ] } as unknown as EditorWidget;
    case 'launcher': return { ...base, label: 'App Launcher', name: 'Calculator', path: 'calc', icon: '📱' } as unknown as EditorWidget;
    case 'clock': return { ...base, label: 'Clock', format24h: true, showSeconds: true, showAmpm: true, showWeekday: false, clockDisplay: 'digital' } as unknown as EditorWidget;
    case 'date': return { ...base, label: 'Date', dateFormat: 'YYYY-MM-DD WWW', showLunar: true } as unknown as EditorWidget;
    case 'calendar': return { ...base, label: 'Calendar', viewMode: 'month', highlightToday: true, gridW: 2, gridH: 3 } as unknown as EditorWidget;
    case 'card': return { ...base, label: '', cardTitle: '', cardDesc: 'Card description', cardImage: '', cardImagePosition: 'top', cardTags: [], cardFooter: '' } as unknown as EditorWidget;
    case 'battery': return { ...base, label: 'Battery', batteryStyle: 'bar', showLevel: true, showCharging: true, showTemp: false, barColor: '#52c41a', lowColor: '#ff4d4f', lowThreshold: 20, dataSource: 'system.battery.level' } as unknown as EditorWidget;
    case 'weather': return { ...base, type: 'weather', label: 'Weather', url: '', showScrollbar: true, backgroundColor: '#ffffff', displayMode: 'webpage', rssUrl: '', jsonUrl: '', refreshInterval: 0, titleField: 'title', descField: 'description', timeField: 'pubDate', linkField: 'link', preset: 'weather', bilibiliRoomId: '', city: '', apiKey: '', unit: 'c', refreshHours: 1, requestMethod: 'GET', requestUrl: 'https://www.pynen.com/api/public/weather?q=${city}&f=wttr', requestHeaders: {}, extraParams: {}, responseMapping: { city: 'nearest_area[0].areaName[0].value', temp: 'current_condition[0].temp_C', condition: 'current_condition[0].weatherDesc[0].value', description: 'current_condition[0].weatherDesc[0].value', icon: 'current_condition[0].weatherCode', humidity: 'current_condition[0].humidity', wind_speed: 'current_condition[0].windspeedKmph', feels_like: 'current_condition[0].FeelsLikeC', forecast: 'weather' } } as unknown as EditorWidget;
...
export function themeToEditor(t: ThemeMeta): EditorTheme {
  return {
    ...
    pages: t.pages.map(p => ({
      ...
      // transport layer 'tile' → editor 'repeat'，same semantics on both sides（tile）
  ...
  return {
    ...
    pages: t.pages.map(p => ({
      ...
      layout: {
        ...
        // the editor uses 'repeat', transport/mobile layer uses 'tile': normalize on export so the phone can recognize it
...
export interface EditorState {
  ...
  redo: () => void;
  clearHistory: () => void;
  // for AI calls: initialize history (clear stack), push snapshot
...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  replaceTheme: (t) => {
    const themeId = t.id;
    // after full replacement: clear selection, clamp active page to valid range（AI full-page generationmay have fewer pages，
    // if the old activePageIdx is kept, out-of-range index leaves the canvas blank, looks like 'nothing happened')
  ...
  initNewTheme: () => {
    const newTheme = { id: genId('theme'), name: 'New Theme', version: '1.0.0', author: '', description: '', source: 'local', pages: [{ ...EMPTY.pages[0], id: genId('page') }] };
    set({
      theme: newTheme,
  ...
  toggleOrientation: () => set(s => {
    ...
    // also write to theme.orientation，so save/export/AI set_orientation all persist
  ...
  saveVersion: (changelog?) => {
    ...
    // estimate snapshot size，skip when base64 image content is too large（skip if over 100KB）
    ...
    // persist to localStorage（try-catch prevent QuotaExceededError）
    if (s.theme.id) {
      try {
  ...
  rollbackToVersion: (idx) => {
    ...
    const ver = hist[idx];
    if (ver) {
      // replace editor state with snapshot
