...
export default function Settings() {
  ...
  const handleLanguageFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      try {
        ...
        // compatible with two formats：{code,label,resources} or {code,label,common:{}}
        ...
        if (json.resources && typeof json.resources === 'object') {
          ...
          // flat format：extract all non-meta fields as common
  ...
  const handleFetchAIModels = async () => {
    ...
    try {
      ...
      let ids: string[];
      if (isTauri()) {
        // forwarded via the Tauri Rust backend，bypass CORS
  ...
  const handleDevSave = () => {
    // parse the full URL，split host and port
  ...
  return (
    <div className="page-container">
          ...
          <Form.Item label={t('settings.ai_models_label')} required>
            <Space orientation="vertical" style={{ width: '100%' }} size={8}>
              ...
              <Form.Item name="models" noStyle rules={[{ required: true, message: t('settings.ai_models_required') }]}>
                <Input.TextArea rows={4} placeholder={'model-a\nmodel-b\nmodel-c'} />
              </Form.Item>
            </Space>
          </Form.Item>
      ...
      {/* developer password verification modal */}
      <Modal
