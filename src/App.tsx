...
import { ConfigProvider, App as AntApp, theme as antTheme, Modal, Button, Typography, Space, Spin } from 'antd';
...
export default function App() {
  ...
  // startup version check（runs once，prevents StrictMode double-call）。
  // when online, keep version/announcement gates; on check failure (backend down or offline), allow entry immediately
  // does not block startup——version polling runs in the background as a safety net。
  useEffect(() => {
    ...
    function doCheck() {
      ...
      clientVersionApi.check(platform, APP_VERSION)
        .then((resp) => {
        ...
        .catch(() => {
          // backend down / network failure：does not block startup，enter the app directly
          allowEntry();
        });
    }
    ...
    // safety fallback：if the request hangs over 8s, allow entry，avoid long blocking
    fallbackTimer = setTimeout(() => { allowEntry(); }, 8000);
  ...
  // detect first launch，show onboarding modal
  useEffect(() => {
    if (allowed) {
  ...
  return (
    ...
    >
      <AntApp>
        ...
        {/* transition page：shown after splash ends but before version check passes，prevents a blank screen */}
        {splashDone && !allowed && !gate && (
          <div
            style={{
