...
/** parse existing theme id from error message ID，e.g. "Theme 'xxx' already exists" */
function parseExistingThemeId(err: string): string | null {
  const match = err.match(/Theme '([^']+)' already exists/);
  return match ? match[1] : null;
}

/** if the target theme exists, prompt for overwrite confirmation，delete old package then re-import after confirmation。return whether the import actually succeeded。 */
...
export const useDownloadStore = create<DownloadState>((set, get) => ({
  ...
  importTheme: async (themeId) => {
    ...
    if (!item) throw new Error('download record not found');
    set({ importing: true });
    try {
  ...
  importAll: async () => {
    ...
    try {
      ...
      if (failedItems.length === 0) {
        ...
        // keep failed items, remove successful ones
