...
export function buildSystemPrompt(theme: EditorTheme, activePageIdx: number, droppedImages?: { name: string; fileName: string }[]): string {
  ...
  return [
    ...
    '```',
    '',
    '注意：widget 的 type 必须是以下之一：button, gauge, snippet-list, image, icon, text, shape, webview, media-control, system-monitor, quick-action, launcher, clock, date, calendar。请根据用户需求生成合适的组件，不要生成不存在的组件类型。',
  ].join('\n');
}